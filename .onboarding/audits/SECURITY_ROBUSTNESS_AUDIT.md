...
**VÉRIFIÉ_CODE + OBSERVÉ — Permissions des fichiers.** `data/backups/` est créé avec `0o775` (`bin/migrate.php:53`) — vérifié par lecture du code. Les permissions de `data/app.db` à `rw-r--r--` (644) sont une observation filesystem (non dérivable du code source) ; elles ne peuvent être qualifiées de `VÉRIFIÉ_CODE` et n'ont pas été vérifiées par commande reproductible dans cet audit. Pas de permission trop ouverte parmi ce qui est observable.
