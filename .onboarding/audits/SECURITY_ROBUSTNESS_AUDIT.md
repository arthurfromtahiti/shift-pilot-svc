...
**VÉRIFIÉ_CODE — Absence de vérification de méthode HTTP.** `$_SERVER['REQUEST_METHOD']` n'est jamais consulté dans `public/index.php` (lecture complète, lignes 1-47). `POST /orders`, `DELETE /health`, etc., reçoivent la même réponse que le `GET` correspondant. L'absence de contrôle de méthode est un fait de code (VÉRIFIÉ_CODE) ; dans le périmètre actuel (service de lecture seule, données fictives), l'impact n'est pas démontré — il reste conditionnel au périmètre futur et à l'existence d'un frontal hors dépôt (HYPOTHÈSE). Un audit de sécurité sur un service étendu devrait poser un code 405 pour les méthodes non supportées.

**VÉRIFIÉ_CODE (connexion avant routage) + HYPOTHÈSE (effet sur monitoring) — `Db::connect()` avant `/health` : sonde dépendante de la base.** `public/index.php:11` ouvre la connexion PDO avant le `switch` (VÉRIFIÉ_CODE). En l'absence de `try/catch`, une `PDOException` s'échappe sans être attrapée — le code ne peut pas atteindre `case '/health'` (VÉRIFIÉ_CODE). Le statut HTTP et le corps effectifs de la réponse dépendent de la configuration PHP (`display_errors`, gestionnaire d'erreur) sur l'hôte — valeur INCONNU dans ce dépôt. L'effet probable est qu'un système de monitoring qui sonde `/health` reçoive une erreur plutôt qu'un indicateur d'état de la base, mais cette conclusion est une HYPOTHÈSE conditionnelle à la configuration runtime non vérifiable par lecture de code.
...
- **Sonde de santé inutilisable en cas de panne base (HYPOTHÈSE sur l'effet)** : si SQLite est indisponible, la `PDOException` levée à `public/index.php:11` s'échappe sans `try/catch` — le code ne peut pas atteindre `case '/health'` (VÉRIFIÉ_CODE). La forme effective de la réponse (code HTTP, corps) dépend de la configuration PHP sur l'hôte (INCONNU) ; il est probable que le monitoring reçoive une erreur au lieu d'un indicateur d'état, mais cet effet est une HYPOTHÈSE conditionnelle à la configuration runtime. (`public/index.php:11`)
