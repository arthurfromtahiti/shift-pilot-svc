...
Les quatre endpoints sont implémentés et cohérents avec la carte des domaines et les workflows. Les chemins de code couvrent les cas d'erreur standard (404 sur commande absente, 404 sur route inconnue) — le comportement HTTP effectif (codes de réponse, rendu sous panne SQLite) n'a pas été observé, faute d'exécution dans ce dépôt (INCONNU). L'unique point fonctionnel conditionnel est le mécanisme de dépôt de `deployed-version.json` : `deploy.yml` (VÉRIFIÉ_CODE) écrit `deployed/<env>/version.json` mais n'écrit pas ce fichier à la racine du projet servi ; `public/index.php:16-19` (VÉRIFIÉ_CODE) lit `deployed-version.json` à cette racine. Si aucun mécanisme externe (hébergement, webhook) ne comble cet écart de chemin, `/version` retournerait `sha: null, ref: null, deployedAt: null` — HYPOTHÈSE conditionnelle, non vérifiable dans ce dépôt (hôte : INCONNU). Un second point à surveiller est la subtilité de l'opérateur `+` sur `/version` : si `deployed-version.json` contient un `schemaVersion` obsolète, la valeur retournée par l'API peut ne pas refléter l'état réel de la base. La cohérence des chemins de code est bonne ; le comportement runtime reste non observé.

## Constats détaillés

**VÉRIFIÉ_CODE (chemin) + HYPOTHÈSE (effet) + INCONNU (hôte) — `GET /health` : chemin implémenté, comportement sous panne SQLite non observé.** `public/index.php:11` ouvre la connexion PDO avant tout routage (VÉRIFIÉ_CODE). `public/index.php:22-24` retourne `{"status":"ok"}` si l'exécution atteint ce point (VÉRIFIÉ_CODE). Aucun `try/catch` n'encadre la connexion (VÉRIFIÉ_CODE). HYPOTHÈSE : si SQLite est inaccessible, une `PDOException` non attrapée s'échapperait, et la réponse serait probablement HTTP 500 — mais le code de réponse effectif dépend de la configuration PHP sur l'hôte (`display_errors`, gestionnaire d'erreurs), qui est INCONNU dans ce dépôt. Fonctionnellement, la sonde teste implicitement la disponibilité de la base. Ce comportement n'est pas documenté comme tel dans le README.

**VÉRIFIÉ_CODE — `GET /version` : chemin implémenté avec un comportement de priorité non trivial.** `public/index.php:27` retourne `$version + ['schemaVersion' => Db::schemaVersion($pdo)]`. L'opérateur `+` PHP conserve la valeur du tableau gauche si la clé existe déjà. `deploy.yml:46` calcule `$SCHEMA` via PHP CLI ; cette valeur est insérée dans le JSON à `deploy.yml:61` (`"schemaVersion": $SCHEMA`) lors de chaque déploiement — donc, si le fichier est présent, c'est la valeur du fichier qui est retournée, pas la base live. Ce comportement n'est pas documenté comme intentionnel dans le code ou le README (HYPOTHÈSE : la valeur du fichier prime pour que la version déployée fasse foi — non prouvé par les sources disponibles) ; il peut induire une divergence silencieuse si la base est modifiée hors déploiement. Si `deployed-version.json` est absent, les champs `sha`, `ref`, `deployedAt` valent `null` et `schemaVersion` est lue depuis la base — comportement gracieux, mais non alertant. Le comportement HTTP effectif (code de réponse, rendu JSON) reste INCONNU sans exécution.

**VÉRIFIÉ_CODE — `GET /orders` : chemin implémenté.** `src/Orders.php:14-16` retourne `SELECT id, client, montant_cents, devise, statut FROM orders ORDER BY id` — liste complète triée. La migration `migrations/001_init.sql:15-20` insère 5 lignes fictives (VÉRIFIÉ_CODE). Le test `testListeToutesLesCommandes` vérifie `count == 5` sur une base temporaire reconstruite depuis ces migrations, pas depuis `data/app.db` (`tests/OrdersTest.php:25-27`, VÉRIFIÉ_CODE). L'état actuel de `data/app.db` n'a pas été inspecté dans cet audit (INCONNU). Le comportement HTTP effectif de la route (code de réponse, rendu JSON) reste INCONNU sans exécution. Pas de pagination, pas de filtre — attendu pour un banc d'essai.

**VÉRIFIÉ_CODE — `GET /orders/{id}` : chemin implémenté avec 404 selon le code.** `public/index.php:35-43` + `src/Orders.php:18-24`. La regex `#^/orders/(\d+)$#` filtre les identifiants non entiers (→ 404 Route inconnue, pas 404 Commande introuvable). `Orders::find()` retourne `null` sur `fetch() === false` et le routeur émet `http_response_code(404)` + `{"error":"Commande introuvable"}`. Test : `testTrouveUneCommandeParIdentifiant` et `testIdentifiantInconnuRenvoieNull` (`tests/OrdersTest.php:29-37`).
...
**VÉRIFIÉ_CODE — Méthodes HTTP non filtrées : comportement défini mais non documenté.** `$_SERVER['REQUEST_METHOD']` n'est jamais consulté (`public/index.php:1-47`). `POST /orders` et `DELETE /health` reçoivent les mêmes réponses que leurs équivalents GET. Dans le périmètre actuel (lecture seule, données fictives), l'impact n'est pas démontré — il reste conditionnel au périmètre futur et à l'existence d'un frontal non décrit dans ce dépôt (HYPOTHÈSE). Le workflow SERVICE documente ce point comme risque mineur (`WORKFLOW_SERVICE.md:94`).
...
**VÉRIFIÉ_CODE — Pas de fonctionnalité inachevée dans le code inspecté.** Le périmètre déclaré (4 endpoints, lecture seule, données fictives) est entièrement implémenté selon le code. Aucune route commentée, aucun `TODO` dans le code source, aucun endpoint sans implémentation dans le code inspecté (`public/index.php:21-47`). La non-régression runtime n'est pas prouvée. Le projet est cohérent avec son objectif de banc d'essai.

**VÉRIFIÉ_CODE (configuration) + INCONNU (exécution) — Distinction staging/production.** `deploy.yml:40` détermine l'environnement dans le code : `main` → `production`, toute autre branche déclencheuse → `staging` (VÉRIFIÉ_CODE). Les deux fichiers `version.json` coexistent sur `deployed` sans s'écraser (correction `da34e1e`, VÉRIFIÉ_CODE). La promotion `staging → production` est explicitement un geste humain hors chaîne d'agents (README, carte des domaines). Aucun run ID ni résultat CI n'a été observé dans cet audit — l'exécution effective de cette logique reste INCONNU.
...
- Distinction staging/production implémentée dans la configuration (`deploy.yml:40`, VÉRIFIÉ_CODE) — exécution effective non observée (INCONNU).
